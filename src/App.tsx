...
function App() {
  ...
  const findUpcoming = (array: any) => {
    ...
    switch (Number(upcoming)) {
      case 1: {
        array.filter((el: any) => {
          //console.log(`el.rocket.rocket_name`, el.rocket.rocket_name)
          switch (String(el.upcoming) == "true") {
            case true:
              upcomings.push(el);
          }
        });
  ...
  const findInterval = (array: any) => {
    ...
    // dispatch(doSomething(_interval));
  ...
  // if(search || upcoming || status ||interval ){
  //   console.log('dbsfghhjfbyfvbdsvbfygdy')
  // useEffect(()=>{if(search || upcoming || status ||interval ){
  //   console.log('dbsfghhjfbyfvbdsvbfygdy')
  //   dispatch(getPosts());
  // }
